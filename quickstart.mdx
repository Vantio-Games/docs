...
const response = await fetch(
  'http://localhost:3000/api/v1/posters?programId=prog_123abc',
  {
    headers: {
      'Authorization': `Bearer ${API_KEY}`
    }
...
const referrals = await fetch(
  'http://localhost:3000/api/v1/referrals?programId=prog_123abc',
  {
    headers: {
      'Authorization': `Bearer ${API_KEY}`
    }
...
const earnings = await fetch(
  'http://localhost:3000/api/v1/earnings?programId=prog_123abc&status=paid',
  {
    headers: {
      'Authorization': `Bearer ${API_KEY}`
    }
